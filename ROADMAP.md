...
* **Evaluate a `mimalloc` backend for `cwist_alloc`/the epoch-GC heap** (tracked in #25): the-benchmarker's C1M-reactor P99.999 numbers still trail Axum by 2.8–3.6x while RPS and average latency are close, which points at rare allocator-tail stalls (glibc arena lock contention, `malloc_consolidate`) rather than a constant per-request cost. Since v3.5 already touches `src/core/mem/alloc.c` and adds `malloc`-interception plumbing (`-Wl,--wrap=malloc`) for the full-GC work above, swapping the underlying allocator behind that same seam is a natural fit rather than a separate effort. Plan: `LD_PRELOAD` A/B against jemalloc and tcmalloc first to confirm the allocator hypothesis and compare candidates, then vendor mimalloc (MIT, small footprint, thread-local segments — best fit for the many-small-short-lived-allocations shape of a request/response lifecycle) if the data holds up.
