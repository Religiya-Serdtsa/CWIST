...
def render() -> None:
    ...
    cwist_c1m_mimalloc_lat_part = get_lat_part("cwist_c1m_mimalloc")
    ...
    ws_summary = (
        ...
        f"- **CWIST (C1M reactor, mimalloc)** — experimental, measured worse than glibc malloc for this mode, see issue #25: {ws_latest.get('cwist_c1m_mimalloc_rps',0):.0f} req/s | Latency {ws_latest.get('cwist_c1m_mimalloc_lat_ms',0):.2f}ms{cwist_c1m_mimalloc_lat_part} | RSS {ws_latest.get('cwist_c1m_mimalloc_rss_kib',0):.0f}KiB | Csw {ws_latest.get('cwist_c1m_mimalloc_csw',0):.0f}\n"
